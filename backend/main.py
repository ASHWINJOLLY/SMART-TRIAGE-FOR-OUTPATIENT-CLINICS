...
app = FastAPI(title="SYMPTORA Backend")
...
async def broadcast_queue():
    """Broadcast current queue to all connected WebSocket clients."""
    sort_queue()
    payload = queue_items
    dead = []
...
async def _broadcast_loop():
    """Heartbeat: push queue updates every 5s to keep WebSocket alive."""
    while True:
...
async def startup_event():
    sort_queue()
    app.state.status_task = asyncio.create_task(update_statuses())
...
@app.get("/health")
async def health():
    return {"status": "ok", "queueCount": len(queue_items), "connections": len(active_connections)}
...
async def triage_patient(data: TriageInput):
    ...
    QUEUE_COUNTER += 1
    token = f"EMG-{random.randint(1000, 9999)}" if triage_level == "critical" else f"AYM-{random.randint(1000, 9999)}"
...
async def add_simulated_patient():
    ...
    complaints = ['High fever', 'Stomach pain', 'Headache', 'Skin rash with itching']
    ...
    token = f"AYM-{random.randint(1000, 9999)}"
    time_str = datetime.now().strftime("%I:%M %p")
...
async def get_livekit_token(language: str = "en-IN"):
    room_id = str(uuid.uuid4())[:8]
    room_name = f"symptora-voice-{room_id}"
...
async def voice_triage(
    ...
    if result.get("triage_data"):
        ...
        token = td.get("token", f"AYM-{random.randint(1000,9999)}")
        time_str = datetime.now().strftime("%I:%M %p")
...
async def get_greeting(language: str = "en-IN"):
    from sarvam_tts import text_to_speech
    greeting_text = GREETINGS.get(language, GREETINGS["en-IN"])
...
OCR_TRIAGE_PROMPT = """You are a medical assistant at a hospital.
