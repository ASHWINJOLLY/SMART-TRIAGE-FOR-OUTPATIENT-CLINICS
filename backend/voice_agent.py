...
VOICE_TRIAGE_PROMPT = """You are SYMPTORA, an AI medical triage assistant at a hospital.
...
GREETINGS = {
    "en-IN": "Hello! I'm SYMPTORA, your AI assistant. Please describe your symptoms.",
    "hi-IN": "नमस्ते! मैं सिम्प्टोरा हूँ, एआई सहायक। आप अपने लक्षण बताइए।",
    "bn-IN": "নমস্কার! আমি সিম্প্টোরা। আপনার লক্ষণগুলি বলুন।",
    "ta-IN": "வணக்கம்! நான் சிம்ப்டோரா. உங்கள் அறிகுறிகளை சொல்லுங்கள்.",
    "te-IN": "నమస్కారం! నేను సింప్టోరా. మీ లక్షణాలను చెప్పండి.",
    "mr-IN": "नमस्कार! मी सिम्प्टोरा आहे. कृपया तुमची लक्षणे सांगा.",
}
...
async def run_gemini_triage(patient_text: str, language: str) -> dict:
    ...
    try:
        ...
        if not triage_data:
            ...
            danger_words = ['chest pain', 'seene', 'dard', 'saans', 'breathing', 'breathless', 'unconscious', 'behosh', 'heart attack', 'accident', 'poison', 'choking', 'paralysis', 'fits',
...
async def add_patient_to_queue(name: str, age: int, gender: str, complaint: str, triage_data: dict) -> dict | None:
    ...
    try:
        token = f"EMG-{random.randint(1000,9999)}" if triage_data["triage_level"] == "critical" else f"AYM-{random.randint(1000,9999)}"
...
async def process_voice_triage(audio_bytes: bytes, language: str = "en-IN") -> dict:
    """
