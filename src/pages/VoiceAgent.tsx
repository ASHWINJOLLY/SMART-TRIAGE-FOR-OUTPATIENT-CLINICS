...
export default function VoiceAgent() {
  ...
  const startSession = async () => {
    ...
    try {
      ...
      let greetingText = lang === 'en-IN' 
        ? 'Hello! I\'m SYMPTORA, your AI assistant. Please describe your symptoms.'
        : 'नमस्ते! मैं सिम्प्टोरा हूँ, एआई सहायक। आप अपने लक्षण बताइए।';
      ...
      setTimeout(() => {
        ...
        addEntry(lang === 'en-IN' ? 'Press the mic button and speak...' : 'Mic button dabayein aur bolein...', 'system');
      }, 2500);
  ...
  const startRecording = async () => {
    try {
      ...
      (window as any).__symptoraPcmChunks = pcmChunks;
      (window as any).__symptoraProcessor = processor;
  ...
  const stopRecordingAndProcess = async () => {
    ...
    const processor = (window as any).__symptoraProcessor;
    if (processor) {
    ...
    const pcmChunks = (window as any).__symptoraPcmChunks as Float32Array[] | undefined;
    if (!pcmChunks || pcmChunks.length === 0) {
      addEntry(lang === 'en-IN' ? 'Audio capture failed. Please try again.' : 'Audio capture nahi hua. Dubara try karein.', 'system');
      setVoiceState('listening');
      return;
    }
    ...
    if (audioBlob.size < 5000) {
      addEntry(lang === 'en-IN' ? 'Audio too short. Please speak longer.' : 'Audio bahut chhota hai. Zyada der tak bolein.', 'system');
      setVoiceState('listening');
      return;
    }

    addEntry(lang === 'en-IN' ? 'Processing your voice...' : 'Aapki awaaz process ho rahi hai...', 'system');
    ...
    try {
      ...
      addEntry(lang === 'en-IN' ? 'Technical issue. Please try again.' : 'Technical issue. Kripya dubara try karein.', 'system');
      setVoiceState('listening');
    }
  };
        ...
        {voiceState === 'idle' ? (
          <>
            ...
            <p className="va-mic-label">
              {lang === 'en-IN' ? 'Start Voice Triage' : 'Voice Triage shuru karein'}
            </p>
            <p className="va-mic-sub">
              {lang === 'en-IN' ? 'Press mic, speak symptoms, AI will triage' : 'Mic dabayein, apne symptoms bolein, AI triage karega'}
            </p>
          </>
          ...
          <>
            ...
            <p className="va-mic-label">Connecting to SYMPTORA...</p>
          </>
        ) : voiceState === 'error' ? (
          <>
      ...
      <div className="va-actions">
        {voiceState === 'idle' ? null : (
          <>
              ...
              <PhoneOff size={16} />
              {lang === 'en-IN' ? 'End Session' : 'Session Band Karein'}
            </button>
            <button className="va-reset-btn" onClick={resetSession}>
              <RotateCcw size={16} />
              {lang === 'en-IN' ? 'New Session' : 'Naya Session'}
            </button>
          </>
        )}
      </div>
